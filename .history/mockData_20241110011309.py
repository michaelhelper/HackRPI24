...
for device_id in range(1, num_devices + 1):
    ...
    # Determine neighbor's initial sound level (previous device or next if first device)
    if device_id == 1:
        neighbor_level = initial_sound_levels[device_id]  # Neighbor is next device
    else:
        neighbor_level = initial_sound_levels[
            device_id - 2
        ]  # Neighbor is previous device
    ...
    while current_time <= datetime.now():
        ...
        # Update neighbor level every 5 minutes (next device's level)
