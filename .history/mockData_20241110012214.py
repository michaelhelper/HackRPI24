...
# Function to calculate approximate distance between two locations
def calculate_distance(loc1, loc2):
    # Approximate conversion of latitude and longitude differences to meters
    lat_diff = (loc2["latitude"] - loc1["latitude"]) * 111000  # 111 km per degree latitude
    lon_diff = (loc2["longitude"] - loc1["longitude"]) * 85000  # ~85 km per degree longitude in Troy, NY
    return math.sqrt(lat_diff**2 + lon_diff**2)

# Function to find the nearest 3 neighbors based on location
def get_nearest_neighbors(device_index, locations):
    distances = []
    for i, location in enumerate(locations):
        if i != device_index:
            dist = calculate_distance(locations[device_index], location)
...
locations = [random_location() for _ in range(num_devices)]  # Generate fixed locations for each device
initial_sound_levels = [random.randint(INITIAL_MIN_SOUND_LEVEL, INITIAL_MAX_SOUND_LEVEL) for _ in range(num_devices)]
neighbor_indices = [get_nearest_neighbors(i, locations) for i in range(num_devices)]  # Nearest 3 neighbors for each device
...
for device_id in range(1, num_devices + 1):
    ...
    while current_time <= datetime.now():
        ...
        neighbor_levels = [initial_sound_levels[neighbor_id] if current_time == start_time else devices_data[-(num_devices - neighbor_id)]['sound_level']
