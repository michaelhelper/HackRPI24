...
def calculate_distance(loc1, loc2):
    ...
    ) * 111000  # 111 km per degree latitude
    lon_diff = (
        loc2["longitude"] - loc1["longitude"]
    ) * 85000  # ~85 km per degree longitude in Troy, NY
...
for device_id in range(1, num_devices + 1):
    ...
    while current_time <= datetime.now():
        ...
        if current_time == start_time:
            # Use initial sound levels for the first entry
            neighbor_levels = [
                initial_sound_levels[neighbor_id]
                for neighbor_id in neighbor_indices[device_id - 1]
            ]
        else:
            # Use the latest sound level from each neighbor device data
            neighbor_levels = [
                devices_data[neighbor_id * len(device_data) - 1]["sound_level"]
                for neighbor_id in neighbor_indices[device_id - 1]
            ]
