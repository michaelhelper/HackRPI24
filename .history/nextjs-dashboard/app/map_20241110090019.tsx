...
// Get user's current timestamp
const userCurrentTimestamp = Date.now();

// Find the closest timestamp in data to the user's current time
const closestTimestamp = timestamps.reduce((prev, curr) => {
  return Math.abs(curr - userCurrentTimestamp) < Math.abs(prev - userCurrentTimestamp) ? curr : prev;
}, timestamps[0]);

// State to manage the selected timestamp, initialized to the closest timestamp
const [selectedTimestamp, setSelectedTimestamp] = useState<number>(closestTimestamp);

// Update selectedTimestamp if the closest timestamp changes
useEffect(() => {
  setSelectedTimestamp(closestTimestamp);
}, [closestTimestamp]);
